 ...
 *   - 操作者を auto 記録
 *
 * AdminPage / WaitingCastPage / AttendanceManager の 3 箇所で同じ UI を
 * 出すため、共有コンポーネントに切り出した。呼び出し側は `onSubmit` で
 * 完成した DailyPayRequest を受け取って `addDailyPayRequest` する責務。
 *
...
export default function DailyPayDialog({
  ...
  const isAdjusted = Number.isFinite(amt) && amt !== calculatedAmount
  // 重大1 + 注意2: 自動計算と異なる場合は調整理由を必須にする。
  const canSubmit =
    ...
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="bg-gray-900 border border-white/10 rounded-lg p-4 max-w-md w-full space-y-3">
        ...
        <div className="text-xs text-gray-400 tabular-nums">
          対象日 {targetDate} / 自動計算額: ¥{calculatedAmount.toLocaleString()}
        </div>
        <div>
